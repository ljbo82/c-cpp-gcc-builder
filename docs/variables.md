...
Some variables are expected to be defined exclusively on any or your project's makefiles, some of them are recommended to be defined on command line, and others are read-only (i.e. contains utility information generated by the build system and cannot be modified by the user/project). The build system will check the origin and the value of each known variable, and when a violation is detected it will generate an error.

!!! Note
    The build system automatically includes the following makefiles (which also expect/expose their own set of variables):
